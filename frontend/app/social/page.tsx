  ...
  return (
    <div className="min-h-screen mesh-bg p-8">
      <div className="max-w-2xl mx-auto">
        ...
        {loading ? (
          <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="shimmer rounded-xl h-14"/>)}</div>
        ) : (
          <>
            {tab === "friends" && (
                ...
                <div className="glass rounded-xl p-4 border border-white/5">
                  ...
                  {searchResults.length > 0 && (
                    <div className="mt-3 space-y-2">
                      {searchResults.map(u => (
                        <div key={u.id} className="flex items-center justify-between glass-light rounded-lg px-3 py-2.5">
                          <div>
                    ...
                    {friends.map(f => (
                      <div key={f.id} className="flex items-center justify-between glass rounded-xl px-5 py-3.5 border border-white/5">
                        ...
                        <button onClick={() => router.push("/interview")}
                          className="text-xs bg-brand-600/15 border border-brand-500/25 text-brand-400 px-3 py-1.5 rounded-lg hover:bg-brand-600/25">
                          Challenge
                        </button>
                      </div>
                    ))}
              ...
              <div>
                {pending.length === 0 ? (
                  <div className="glass rounded-2xl p-10 border border-dashed border-white/8 text-center">
                    ...
                    <p className="text-xs text-slate-600">Incoming friend requests appear here</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {pending.map(req => (
                      <div key={req.friendship_id} className="flex items-center justify-between glass rounded-xl px-5 py-3.5 border border-white/5">
                        <div>
                    ...
                    {groups.map(g => (
                      <div key={g.id} className="flex items-center justify-between glass rounded-xl px-5 py-3.5 border border-white/5">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{g.topic || "General"} · {g.member_count} members</p>
                        </div>
